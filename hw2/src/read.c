...
char *tokenptr;
char *tokenend;
char *memerr2 = "Unable to allocate memory.";
Course *readfile(root)
char *root;
{

        if((newptr = malloc(32)) == NULL) {
            fatal(memerr2);
        }
        tokenptr = tokenend = newptr;
        Course *c;
...
void advancetoken()
{
        ...
        while((c = getc(ifile->fd)) != EOF) {
                ...
                int newend = tokensize();
                if(newend >= (32 + sizealloc)) {
                  sizealloc += 32;
                  if((newptr = realloc(newptr, 32 + sizealloc)) == NULL) {
                    exit(1);
                  }
                  tokenptr = newptr;
                  tokenend = tokenptr + newend;
                }
                *tokenend++ = c;
        }
